...
def  generate_row_attr(args):
    """Converts the gene metrics from the Optimus pipeline to row attributes for h5ad file
...
def generate_col_attr(barcode_1_path,barcode_2_path,cell_metrics_path):
    """Converts cell metrics from the Optimus pipeline to h5ad file
...
def create_h5ad_files(args):
    """This function creates the loom file or folder structure in output_h5ad_path in format file_format,
...
# add input_id to col_attrs
   ...
   # import the expr_sp_t and exon_sp_t matrices as CSR and transpose
    ...
    exon_counts = csr_matrix(exon_sp_t).transpose()

    new_data = ad.AnnData(counts)
    print(new_data.X)
    
    # Import the column and row dictionaries as a dataframe
    df=pd.DataFrame.from_dict(col_attrs)
    dv=pd.DataFrame.from_dict(row_attrs)

    # Assign the column and row attributes to the obs and var fields respectively
    new_data.obs=df
    new_data.var=dv

    # Assign global attributes to unstructured data
    new_data.uns=attrDict

    # Set obs_names
    new_data.obs_names = [x for x in new_data.obs["CellID"]]
...
def main():
    description = """This script converts the some of the Optimus outputs into
                   h5ad format.
    ...
    parser.add_argument(
        ...
        dest="gene_id_1",
        required=True,
        help="a .npy file path for the gene ids in the main layer of the h5ad, an output of the MergeCountFiles task",
    )
    ...
    parser.add_argument(
        ...
        dest="count_matrix_1",
        required=True,
        help="a .npz file path for the count matrix for the main layer of the h5ad, an output of the MergeCountFiles task",
    )
    parser.add_argument(
        "--cell_id_2",
        dest="cell_id_2",
    ...
    parser.add_argument(
        ...
        required=True,
        help="path to .h5ad file is to be created",
    )
