...
def create_h5ad_file(args):
    """Creates an h5ad file with input_id from the input folder analysis_output_path
    Args:
        ...
        output_h5ad_path (str): location of the output h5ad
    """
    ...
    adata = anndata.AnnData(X=exon_expr_csr_coo, obs=col_attrs, var=row_attrs)
    adata.layers['intron_counts'] = intron_expr_csr_coo

    adata.uns['pipeline_version'] = args.pipeline_version

    adata.write(args.output_h5ad_path)

def main():
    description = """This script converts some of the SmartSeq2 pipeline outputs into
                   h5ad format (https://anndata.readthedocs.io/en/stable/) relevant output.
