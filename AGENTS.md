...
- `scvi-tools` is pinned via a build `ARG` and `snapatac2` is hardcoded — **keep them pinned**; changing `scvi-tools` (currently 1.5.1) or `snapatac2` (currently 2.10.0) changes model behavior for the consuming pipeline.
